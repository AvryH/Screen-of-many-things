...
export const RegisterForm: React.FunctionComponent<IRegisterProps> = (props) => {
  ...
  let isCust: boolean =  props.isCustomer

  const handleCustomerChange = (e) => {
    console.log(e.target.value)
    isCust =(e.target.value === "customer")?
    true : false
  }
  ...
  const registerNewUser = async (e: SyntheticEvent) => {
    let newUser:User = {
      ...
      lastName,
      email,
      role : (isCust)?"CUSTOMER":"SHOPPER"
    }

    let requestURL:string = (isCust)?"http://localhost:8080/customers/register"
